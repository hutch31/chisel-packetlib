...
class PacketReader(c : BufferConfig, txbuf : Int = 1) extends Module {
  ...
  val txq = Module(new Queue(new PacketData(c.WordSize), txbuf))
  val metaQueue = Module(new Queue(new WordMetadata(c.WordSize), txbuf))
  ...
  val length = Module(new Queue(UInt(log2Ceil(c.MTU).W), 1))
