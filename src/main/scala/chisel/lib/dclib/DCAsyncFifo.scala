...
class DCAsyncFifo[D <: Data](data: D, depth: Int, doubleSync: (UInt) => UInt = defaultDoubleSync) extends RawModule {
  ...
  val wrptr_enq = withClockAndReset(io.enqClock, io.enqReset) {
    RegInit(init = 0.U(ptrsz.W))
  }
  val wrptr_grey_enq = Wire(UInt(ptrsz.W))
  wrptr_grey_enq := BinaryToGray(wrptr_enq)
  ...
  val rdptr_grey_deq = Wire(UInt(ptrsz.W))
  ...
  val full_enq =
    wrptr_enq(asz - 1, 0) === rdptr_enq(asz - 1, 0) & (wrptr_enq(asz) =/= rdptr_enq(asz))
